...
//		this.environment = environment;
	...
	@RequestMapping(path = PATH + "/date/{date}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	...
	public ResponseEntity<Resources<Resource<Course>>> indexCourse(@PathVariable("studentId") @Min(0) Long studentId) {
		...
		Iterable<Course> entities = student.getCoursesList();
...
//		String port = environment.getProperty("server.port");
		...
		if (entity.getStudentsSet() != null) {
			for (Student student : entity.getStudentsSet()) {
				resource.add(new Link(url + StudentController.PATH + "/" + student.getId(), RouteConstant.REL_TO_STUDENTS));
			}
		}
		return resource;
	}
